...
const getResourceUrl = (resource: { url: string; platform: string }, topicName: string): string => {
  // If URL is a placeholder or broken, generate a search URL
  ...
  // For YouTube links, convert to search if needed
  if (resource.platform.toLowerCase() === "youtube" && !resource.url.includes("youtube.com/watch")) {
    return generateYouTubeSearchUrl(topicName);
  }
...
export function CareerRoadmapContent() {
                          ...
                          <CollapsibleContent>
                            <div className="p-4 pt-0 space-y-3">
                              {topic.subTopics.map((subTopic) => {
                                const freeUrl = getResourceUrl(subTopic.freeResource, subTopic.name);
                                const paidUrl = getResourceUrl(subTopic.paidResource, subTopic.name);
